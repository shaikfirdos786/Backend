...
app.get("/jokes/:id", (req, res) => {
  ...
  /* first method to find the specific joke
  ...
  */
  ...
  // second method to find specific joke 
  const joke = jokes.find((joke) => joke.id === jokeId);
  res.json(joke);

  // If the loop completes and no matching joke is found, return an error response
...
app.get("/filter", (req, res) => {
  ...
  if (filteredJokes.length === 0) {
    return res.status(404).json({ error: "No jokes of that type found" });
  }
...
app.put("/jokes/:id", (req, res) => {
  ...
  // Update the existing joke with the new text and type
  jokes[jokeIndex] = {
    id: id,
    jokeText: text,
    jokeType: type,
...
app.delete("/jokes/:id", (req, res) => {
  ...
  // Remove the joke with the specified index from the 'jokes' array
  const deletedJoke = jokes.splice(deleteIndex, 1);

  res.status(200).json({ message: "Joke deleted successfully", deletedJoke});
});
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    ...
    return res
      .status(200)
      .json({ message: "Successfully deleted all the jokes." });
  } else {
